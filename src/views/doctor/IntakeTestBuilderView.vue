<template>
  <div class="p-4 md:p-6 max-w-3xl mx-auto" :dir="locale === 'ar' ? 'rtl' : 'ltr'">
    <IntakeBuilder
      :back-to="backTo"
      :title="form.id ? $t('intakeTests.edit') : $t('intakeTests.add')"
      :loading="loading"
      :saving="saving"
      :form="form"
      :form-error="formError"
      @save="save"
      @update:form-error="formError = $event"
    />
  </div>
</template>

<script setup lang="ts">
import { reactive, ref, onMounted, computed } from 'vue'
import { useI18n } from 'vue-i18n'
import { useRoute, useRouter } from 'vue-router'
import { useQueryClient } from '@tanstack/vue-query'
import { apiFetch } from '@/lib/utils'
import IntakeBuilder, { type IntakeForm, type IntakeQuestion, type AnyOldQuestion } from '@/components/IntakeBuilder.vue'

const { locale } = useI18n()
const route = useRoute()
const router = useRouter()
const qc = useQueryClient()

const loading = ref(false)
const saving = ref(false)
const formError = ref('')

const form = reactive<IntakeForm>({
  id: null,
  name: '',
  description: '',
  is_active: true,
  questions: [],
})

const backTo = computed(() => '/doctor/intake-tests')

async function save(payload: { name: string; description?: string; is_active: boolean; questions: IntakeQuestion[] }) {
  formError.value = ''
  saving.value = true
  try {
    const body = JSON.stringify(payload)
    if (form.id) {
      await apiFetch(`/intake-tests/${form.id}`, { method: 'PATCH', body })
    } else {
      await apiFetch('/intake-tests', { method: 'POST', body })
    }
    qc.invalidateQueries({ queryKey: ['intake-tests'] })
    router.push('/doctor/intake-tests')
  } catch (e) {
    formError.value = (e as Error).message
  } finally {
    saving.value = false
  }
}

interface IntakeTestRow {
  id: string
  name: string
  description?: string | null
  is_active: boolean
  questions: AnyOldQuestion[]
}

onMounted(async () => {
  const id = route.params.id as string | undefined
  if (!id) return
  loading.value = true
  try {
    const list = await apiFetch<IntakeTestRow[]>('/intake-tests')
    const t = list.find((x) => x.id === id)
    if (!t) {
      formError.value = 'Intake test not found'
      return
    }
    form.id = t.id
    form.name = t.name
    form.description = t.description || ''
    form.is_active = t.is_active
    form.questions = (t.questions || []).map((q) => normalizeQuestion(q))
  } finally {
    loading.value = false
  }
})

function uid() {
  return 'q_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 8)
}

function normalizeQuestion(q: AnyOldQuestion): IntakeQuestion {
  let type: IntakeQuestion['type'] = 'text'
  switch (q.type) {
    case 'text':
    case 'single_choice':
    case 'multi_choice':
    case 'guidance':
      type = q.type
      break
    case 'radio':
      type = 'single_choice'
      break
    case 'checkbox':
    case 'select':
      type = 'multi_choice'
      break
    default:
      type = 'text'
  }
  const text = (q.text ?? q.text_ar ?? q.text_en ?? '').toString()
  const options = (q.options || []).map((o) => ({
    id: o.id || uid(),
    text: (o.text ?? o.text_ar ?? o.text_en ?? '').toString(),
  }))
  return {
    id: q.id || uid(),
    text,
    type,
    required: type === 'guidance' ? false : !!q.required,
    options: type === 'single_choice' || type === 'multi_choice' ? options : [],
  }
}
</script>
