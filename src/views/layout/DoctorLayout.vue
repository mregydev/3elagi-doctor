<template>
  <div class="flex h-screen overflow-hidden" style="background: hsl(var(--gray-0))" :dir="locale === 'ar' ? 'rtl' : 'ltr'">
    ...
    >
      ...
      <!-- Doctor name under logo -->
      <div class="px-5 py-3 border-b border-[hsl(var(--border))]">
        <div class="text-xs text-[hsl(var(--muted-foreground))]">{{ $t('auth.doctorTab') }}</div>
...
import { ListOrdered, Building2, LogOut, Menu, Settings, FileText, ClipboardList, CalendarDays, CalendarCheck, Users } from 'lucide-vue-next'
...
const navItems = [
  { to: '/doctor/queue', label: 'nav.queue', icon: ListOrdered },
  { to: '/doctor/patients', label: 'nav.patients', icon: Users },
  { to: '/doctor/reservations', label: 'nav.reservations', icon: CalendarCheck },
  { to: '/doctor/schedule', label: 'nav.schedule', icon: CalendarDays },
  { to: '/doctor/clinics', label: 'nav.clinics', icon: Building2 },
  { to: '/doctor/templates', label: 'nav.templates', icon: FileText },
  { to: '/doctor/intake-tests', label: 'nav.intakeTests', icon: ClipboardList },
  { to: '/doctor/settings', label: 'nav.settings', icon: Settings },
]
